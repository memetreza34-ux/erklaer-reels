...
async function createFixture({ missingFirstPrompt = false, missingSecondPrompt = false, missingExtraPrompt = false } = {}) {
  ...
  if (!missingFirstPrompt) await writeFile(path.join(root, 'scenes', 'scene-01', 'image-prompt.txt'), `${NEUTRAL_BASE} Show one large round countryball-like character performing one clear action. Integrate exactly "ERSTE SZENE".`, 'utf8');
  if (!missingSecondPrompt) await writeFile(path.join(root, 'scenes', 'scene-02', 'image-prompt.txt'), `${NEUTRAL_BASE} Show a large round countryball-like actor with one meaningful prop. Integrate exactly "SZENE ZWEI".`, 'utf8');
  if (!missingExtraPrompt) await writeFile(path.join(root, 'scenes', 'scene-02', 'image-prompt-02.txt'), `${NEUTRAL_BASE} Show one concrete symbolic object close-up in the same world. No readable text.`, 'utf8');
  await writeFile(path.join(root, 'scenes', 'scene-03', 'image-prompt.txt'), `${NEUTRAL_BASE} Show a strong final symbolic scene in the same countryball world. Integrate exactly "DRITTE SZENE".`, 'utf8');
  return root;
}
...
test('README erklärt genau eine Google-Flow-Masterdatei', async () => {
  ...
  assert.match(readme, /eine.*verbindliche Masterdatei/i);
  assert.match(readme, /99-alle-bildprompts\.txt/);
  assert.ok(readme.includes(FIXED_VISUAL_WORLD_LABEL));
});

test('exportiert einen seriellen Gesamtprompt in der aktiven Bildwelt', async () => {
  const root = await createFixture();
  ...
  assert.match(bundle, /countryball/i);
