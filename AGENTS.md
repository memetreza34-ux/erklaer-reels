...
- Standardposition: `safe-lower-middle` bei 77 % der Bildhöhe.
- Erlaubter vertikaler Bereich: 73–79 %.
...
- Ohne gültige `wordTimings` bleibt der komplette Untertitel weiß; eine falsche gelbe Markierung ist schlechter als keine Markierung.

Jeder finale Cue benötigt nach dem Voice-over-Sync:
...
Codex hört das echte Voice-over ab und trägt für jede Szene `cueTimeSeconds` und `confidence` ein. Anschließend hört Codex jeden Untertitel-Cue ab und ergänzt in `subtitles/subtitle-plan.json` die echten absoluten `wordTimings` für jedes sichtbare Wort.
...
Die Vorprüfung blockiert den finalen Render, wenn die gelbe Markierung aktiviert ist, aber exakte `wordTimings` fehlen oder fehlerhaft sortiert sind.
