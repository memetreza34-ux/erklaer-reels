...
test('erstellt standardmäßig ein Ein-Minuten-Reel ohne Untertitel', async () => {
  ...
  const result = await createReelWorkspace({
    ...
    script: 'Dieses Rohscript wird durch Codex zu einem vollständigen Ein-Minuten-Reel ohne Untertitel ausgebaut.',
  ...
  assert.equal(result.reel.subtitlesEnabled, false);
  ...
  assert.equal(subtitlePlan.enabled, false);
  assert.deepEqual(subtitlePlan.cues, []);
  assert.equal(status.subtitles, 'disabled');
  assert.equal(status.wordSync, 'not-required');
});

test('strenge Inhaltsprüfung akzeptiert 12 vollständige Szenen ohne Untertitel', async () => {
  ...
  for (let index = 1; index <= 12; index += 1) {
    ...
    const prompt = `Vertical 9:16 adult hand-drawn 2D editorial cartoon. Show one clear democratic visual moment for scene ${index}, using consistent human characters, thick dark outlines, flat colors and subtle paper texture. Use the full frame naturally. Do not create an empty horizontal band, disconnected halves, repeated main characters, labels, logos, watermark or 3D rendering.`;
