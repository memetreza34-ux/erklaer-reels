...
1. Lies zuerst \`AGENTS.md\`, \`knowledge/production-rules.md\`, \`knowledge/effects-rules.md\`, \`knowledge/subtitle-pacing-rules.md\`, \`config/content-rules.json\`, \`config/effects-rules.json\` und \`config/image-styles.json\`.
...
14. Fülle \`subtitles/subtitle-plan.json\` aus:
    - Standardposition bei 79,5 % der Bildhöhe
    - sichere Zone 76,5–80,5 %
    ...
    - \`highlightCurrentWord\` aktivieren und \`highlightColor\` auf \`#FFD84D\` setzen
    - Timing zunächst schätzen und als noch nicht final markieren, bis die echte Audiodatei vorliegt
    - \`wordTimings\` oder \`words\` erst nach dem echten Voice-over mit absoluten Wortzeiten füllen
    - ohne verifizierte Wortzeiten darf keine gelbe Schätzung angezeigt werden; der Text bleibt dann weiß
...
- Ergänze pro Cue \`wordTimings\` oder \`words\` mit den echten absoluten Start- und Endzeiten jedes sichtbaren Wortes.
...
- Nach einer Pausenkürzung müssen die Wortzeiten erneut geprüft werden.
