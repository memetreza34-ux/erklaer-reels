...
export async function createReelWorkspace({
  ...
  const directories = [
    ...
    'timeline',
    'render',
    'all-image-prompts/individual-prompts',
    'inbox/images',
    'inbox/audio',
    'inbox/processed'
  ];
  ...
  const reel = {
    ...
    visualStyleReason: 'Globale Nutzerregel: Für alle neuen Reels ist ausschließlich die universelle Kugel-Welt aktiv; andere Bildwelten sind pausiert.',
  ...
  await writeJson(path.join(reelDirectory, 'status.json'), {
    ...
    plannedImageCount: sceneCount,
    visualStyle: 'round-country-characters',
    imagePromptDelivery: 'one-file-per-image',
    subtitles: 'disabled',
