...
async function readQualityGates() {
  return readJson(path.resolve('config', 'production-quality-gates.json'), {
    assetMatching: {
      ...
      allowedMatchMethods: ['numbered-global-image-order', 'visual-content-review']
    }
  });
}
...
function validateVisualAssignment(assignment, target, scene, rules) {
  ...
  if (confidence < Number(rules.minimumConfidence ?? 0.75)) {
    return `Konfidenz unter ${rules.minimumConfidence ?? 0.75}`;
  }
  if (rules.requireVisualReview && assignment.visualReviewed !== true) return 'visuelle Prüfung wurde nicht bestätigt';
  if (rules.requireSecondPassConfirmation && assignment.secondPassConfirmed !== true) return 'zweite Zuordnungsprüfung fehlt';
  if (rules.requireMatchReason && String(assignment.reason ?? '').trim().length < Number(rules.minimumMatchReasonLength ?? 0)) {
    return 'Zuordnungsbegründung fehlt oder ist zu kurz';
  }
  if (rules.requireVisibleSummary && String(assignment.visibleSummary ?? '').trim().length < Number(rules.minimumVisibleSummaryLength ?? 0)) {
  ...
  const requiredFields = scene ? (rules.requiredSceneComparedFields ?? []) : [];
  const missingComparedFields = requiredFields.filter((field) => !comparedFields.includes(field));
  if (missingComparedFields.length > 0) return `nicht mit Pflichtfeldern verglichen: ${missingComparedFields.join(', ')}`;
  ...
  if (rules.forbidFilenameOnlyMatching && (!matchMethod || matchMethod === 'filename-only')) {
    return 'Zuordnung nur nach Dateiname/Reihenfolge ist verboten';
  }
  if (Array.isArray(rules.allowedMatchMethods) && rules.allowedMatchMethods.length > 0 && matchMethod && !rules.allowedMatchMethods.includes(matchMethod)) {
    return `nicht erlaubte Zuordnungsmethode: ${matchMethod}`;
  }

  if (scene && rules.requireSceneOrderConfirmation) {
    if (assignment.sceneOrderConfirmed !== true) return 'Szenenreihenfolge wurde nicht bestätigt';
    if (String(assignment.confirmedTarget ?? '').trim() !== target) return `confirmedTarget muss exakt ${target} sein`;
    if (Number(assignment.confirmedSceneOrder) !== Number(scene.order)) return `confirmedSceneOrder muss exakt ${scene.order} sein`;
  }
  return null;
}
...
export async function buildAssetInventory(reelDirectory) {
  ...
  for (let index = 0; index < phases.length; index += 1) {
    ...
    visuals.push(visualInventoryEntry(phase, phases[index - 1]?.targetId ?? null, phases[index + 1]?.targetId ?? null, imagePrompt));
  }
  ...
  const inventory = {
    ...
    instructions: [
      'Bei Bild 01..NN ist die globale Nummernfolge die Hauptautorität.',
      'Ein schneller visueller Spot-Check soll nur offensichtliche Inhalts-, Reihenfolge- oder Stilkonflikte erkennen.',
      'Keine schriftliche Bildbeschreibung, Match-Begründung oder zweite Prüfung pro Bild erforderlich.',
      'Nur echte Konflikte bleiben unmatched.'
    ],
    visuals,
    scenes: sceneIndex,
    candidates: { images: imageFiles, audio: audioFiles }
  };
  ...
  if (!(await exists(mapPath))) {
    await writeJson(mapPath, { version: 5, generatedBy: '', assignments: [], unmatched: [] });
  }
  return inventory;
}
...
export async function applyAssetMap(reelDirectory) {
  ...
  if (!assetMap || !Array.isArray(assetMap.assignments)) throw new Error('inbox/asset-map.json fehlt oder enthält keine assignments-Liste.');
  ...
  for (const assignment of assetMap.assignments) {
    ...
    const confidence = Number(assignment.confidence ?? 0);
    if (!source || !target) { skipped.push({ assignment, reason: 'source oder target fehlt' }); continue; }
    if (usedSources.has(source) || usedTargets.has(target)) { skipped.push({ assignment, reason: 'Quelle oder Ziel wurde doppelt verwendet' }); continue; }

    const sourcePath = path.resolve(inboxDirectory, source);
    ensureInside(path.resolve(inboxDirectory), sourcePath);
    if (!(await exists(sourcePath))) { skipped.push({ assignment, reason: 'Quelldatei nicht gefunden' }); continue; }
    ...
    if (target === 'audio') {
      if (!AUDIO_EXTENSIONS.has(extension)) { skipped.push({ assignment, reason: 'Datei ist kein unterstütztes Audioformat' }); continue; }
      destinationPath = path.join(reelDirectory, 'audio', `voiceover${extension}`);
      expectedRelativePath = `audio/voiceover${extension}`;
      manifest.audio = { expectedFile: expectedRelativePath, source, confidence, status: 'ready' };
      status.audio = 'ready';
    } else if (targets.has(target)) {
      if (!IMAGE_EXTENSIONS.has(extension)) { skipped.push({ assignment, reason: 'Datei ist kein unterstütztes Bildformat' }); continue; }
      ...
      if (visualError) { skipped.push({ assignment, reason: visualError }); continue; }

      verification = {
        routingConfirmed: true,
        visualReviewed: assignment.visualReviewed === true,
        secondPassConfirmed: assignment.secondPassConfirmed === true,
        ...
        matchMethod: String(assignment.matchMethod ?? 'numbered-global-image-order'),
      ...
      const phases = normalizeSceneImagePhases(scene).map((phase) => phase.targetId !== target ? phase : ({
        ...phase,
  ...
  const readyVisualCount = expectedVisualTargets.filter((phase) => manifestVisualByTarget.get(phase.targetId)?.status === 'ready').length;

  status.plannedImageCount = expectedVisualTargets.length;
  status.images = readyVisualCount === expectedVisualTargets.length ? 'ready' : readyVisualCount > 0 ? 'partial' : 'missing';
  status.assetMatching = readyVisualCount === expectedVisualTargets.length ? 'numbered-routing-complete' : 'needs-review';
  ...
  const visualVerification = expectedVisualTargets.map((phase) => {
    ...
    return {
      ...
      passed: manifestVisual?.status === 'ready'
    };
  });
  ...
  const verificationReport = {
    version: 3,
    mode: 'numbered-routing-plus-fast-visual-qc',
    createdAt: new Date().toISOString(),
    passed: visualVerification.length > 0 && visualVerification.every((entry) => entry.passed),
    minimumConfidence: matchingRules.minimumConfidence ?? 0.75,
    visuals: visualVerification,
    scenes: sceneVerification,
    instructions: [
      'Bild 01..NN wird anhand der globalen Nummernfolge geroutet.',
      'Danach check:visuals als schnellen Einmal-Check ausführen.',
      'Keine zweite visuelle Prüfung und keine schriftliche Begründung pro Bild erforderlich.'
    ]
  };
