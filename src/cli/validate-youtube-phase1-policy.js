...
async function main() {
  const dir = arg('--dir');
  if (!dir) {
    throw new Error('Nutzung: npm run validate:youtube-phase1 -- --dir "youtube/<woche>/<thema>"');
  }
  ...
  if (!meta.topicCoreLink || typeof meta.topicCoreLink !== 'string') {
    errors.push('topicCoreLink muss den Bezug zum Kanalfokus kurz dokumentieren.');
  }
  ...
  for (const marker of requiredPromptMarkers) {
    if (!prompt.includes(marker)) errors.push(`Masterprompt fehlt Pflichtmarker: ${marker}`);
  }
  ...
  for (const pattern of stalePatterns) {
    if (pattern.test(prompt)) errors.push(`Masterprompt enthält veraltete Bildwelt-/Referenzregel: ${pattern}`);
  }
  ...
  console.log(`YouTube Phase-1-Policy: BESTANDEN — Visual Policy V${policy.visualPolicyVersion}, Kanalfokus, Session-Schutz und Anti-Lifeless-Regeln sind aktiv.`);
}
